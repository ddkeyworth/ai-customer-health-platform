...
The original `ai-customer-health-platform.vercel.app` domain was accurate but not a name anyone would show a prospective employer. Landed on `bearing360.vercel.app` after a deliberate scoring review of ten candidate names against clarity, tone, and memorability - a free Vercel subdomain, no purchase needed. Added as a second domain on the existing Vercel project (both now resolve to the same production deployment); confirmed genuinely live, not just added in the dashboard, by getting a clean `200` on `/marketing` and the expected `307` (unauthenticated redirect to `/login`) on `/` against the new domain directly. README's three live-link references updated to match; the GitHub repo's Website field needs the same update via the GitHub UI (Claude Code cannot write repo metadata without a token).
